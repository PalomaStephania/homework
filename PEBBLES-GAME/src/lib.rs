...
extern "C" fn init() {
    ...
    assert!(init_data.pebbles_count > 0, "Pebbles count must be positive");
    assert!(init_data.max_pebbles_per_turn > 0, "Max pebbles per turn must be positive");
...
fn program_turn(game_state: &mut GameState) {
    let pebbles_to_remove = match game_state.difficulty {
        DifficultyLevel::Easy => {
            1 + (get_random_u32() % game_state.max_pebbles_per_turn)
        },
...
extern "C" fn handle(){
    ...
    match action {
        PebblesAction::Turn(pebbles) => {
            assert!(pebbles > 0 && pebbles <= game_state.max_pebbles_per_turn, "Invalid move");
...
#[no_mangle]
extern "C" fn state() {
    let game_state = STATE.load().expect("Unable to load game state");
